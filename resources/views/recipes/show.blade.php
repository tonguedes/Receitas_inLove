            ...
            @auth
                <div class="absolute top-4 right-4">
                    <form action="{{ route('recipes.favorite', $recipe) }}" method="POST">
                        @csrf
                        <button type="submit"
                            class="bg-white/80 backdrop-blur-md p-2 rounded-full shadow hover:scale-105 transition">
                            @php
                                $favorited = auth()->user()->favoriteRecipes->contains($recipe->id);
                            @endphp
                            @if ($favorited)
                                {{-- Ícone preenchido --}}
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-pink-600 fill-current"
                                    viewBox="0 0 20 20">
                                    <path
                                        d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 18.343l-6.828-6.829a4 4 0 010-5.656z" />
                                </svg>
                            @else
                                {{-- Ícone só com contorno --}}
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-pink-600 stroke-current"
            ...
            @auth
                <form method="POST" action="{{ route('recipes.comments.store', $recipe->id) }}" class="mb-6">
                    ...
                    <textarea name="body" rows="3"
                        class="w-full border border-gray-300 p-3 rounded-md shadow-sm focus:ring-pink-300"
