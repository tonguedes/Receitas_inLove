        ...
        <div class="py-6 px-4 max-w-7xl mx-auto space-y-6">
            @foreach ($recipes as $recipe)
                <div class="bg-white shadow-md rounded-2xl p-5 flex items-center gap-6 hover:shadow-lg transition-shadow">
                    {{-- Imagem --}}
                    @if ($recipe->image)
                        <img src="{{ asset('storage/' . $recipe->image) }}" alt="Imagem da Receita"
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="flex items-center space-x-2">
                            <form action="{{ route('recipes.aprovar', $recipe->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2 text-sm font-medium bg-green-500 text-white rounded-lg hover:bg-green-600 transition">
                                    Aprovar
                                </button>
                            </form>

                            <form action="{{ route('recipes.rejeitar', $recipe->id) }}" method="POST">
                                @csrf
                                <button type="submit"
